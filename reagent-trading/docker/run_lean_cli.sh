...
# Script to run Lean CLI in Docker for ReAgent Trading System
...
create_strategy() {
    ...
    # Run Lean CLI in Docker to create a project
    docker run --rm \
        -v "$BASE_DIR":/app \
        quantconnect/lean:foundation \
        /bin/bash -c "cd /app && lean create-project strategies/$STRATEGY_NAME -l python"
...
run_backtest() {
    ...
    # Run Lean CLI in Docker to run a backtest
    docker run --rm \
        -v "$BASE_DIR":/app \
        -v "$DATA_DIR":/Data \
        ...
        --algorithm-location=/app/strategies/$STRATEGY_NAME/main.py
